...
// =====================================
// -------- MotorX setup --------
const int stepPinY = 3;   // PUL+ for MotorX
const int dirPinY  = 5;   // DIR+ for MotorX
const int enPinY   = 7;   // ENA+ for MotorX
// -------- MotorY setup --------
const int stepPinX = 22;   // PUL+ for MotorY
const int dirPinX  = 26;   // DIR+ for MotorY
const int enPinX   = 30;   // ENA+ for MotorY

// -------- MotorZ setup --------
const int stepPinZ = 34;   // PUL+ for MotorZ
const int dirPinZ  = 38;   // DIR+ for MotorZ
const int enPinZ   = 42;   // ENA+ for MotorZ
...
const unsigned int stepPulseHighUsX = 10;
const unsigned int stepDelayUsX     = 30;
const unsigned int stepPulseHighUsY = 10;
...
const unsigned int stepPulseHighUsZ = 50;
const unsigned int stepDelayUsZ     = 100;
...
void homeMotorX() {
  ...
  moveMotorX(HIGH, (pulsesPerMMX * 50));
...
void homeMotorY() {
  ...
  moveMotorY(HIGH, (pulsesPerMMY * 54));
  delay(500);
...
void Wash() {
  delay(500);
  moveMotorX(HIGH, (pulsesPerMMX * 300)); //MOVE TO WASH POSITION 
  delay(500);
  moveMotorZ(HIGH, (pulsesPerMMZ * 15)); //MOVE UP TO SONICATE 
  delay(500);
  digitalWrite(sonicator, HIGH); //START SONICATION
  ...
  digitalWrite(sonicator, LOW); //STOP SONICATION
  delay(500); 
  moveMotorX(LOW, (pulsesPerMMX * 300)); //MOVE TO SAMPLE PLATE
  delay(500);
...
void loop() {
  ...
  moveMotorX(LOW, pulsesPerMMZ * 50); //MOVE TO SAMPLE PLATE
  delay(2000);    
  moveMotorX(LOW, pulsesPerMMZ * 50); //MOVE TO SAMPLE PLATE
  delay(2000);
  moveMotorX(LOW, pulsesPerMMZ * 50); //MOVE TO SAMPLE PLATE
  ...
  moveMotorX(LOW, pulsesPerMMZ * 50); //MOVE TO SAMPLE PLATE
  delay(2000);
  moveMotorX(HIGH, pulsesPerMMZ * 250); //MOVE TO SAMPLE PLATE
  delay(2000);
  ...
  if (PROTOCOL96 == LOW) {
    homeMotorZ();
    delay(500);
    moveMotorZ(HIGH, (pulsesPerMMZ * 1)); //MOVE TO SAMPLE PLATE
    homeMotorX();
    delay(500);
    homeMotorY();
    ...
    delay(500);
    moveMotorX(LOW, (pulsesPerMMX * 36));
    Sonicate(150000);
    Wash();
    ...
    Sonicate(150000);
    Wash(); 
    moveMotorX(HIGH, (pulsesPerMMX * 340));  
  }
